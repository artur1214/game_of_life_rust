...
impl Life {
    ...
    fn new(file: impl Into<String>) -> Self {
        ...
        let parsed_field: Vec<Vec<Cell>> = field.lines().skip(1).map(|row| {
            row.chars().map(|c| {
    ...
    fn next(&mut self) {
        ...
        self.field = self.field.iter().enumerate().map(|(row_idx, row)| {
            row.iter().enumerate().map(|(col, cell) |{
                ...
                return match cell {
                    Cell::Dead => if alives==3 {Cell::Alive} else {Cell::Dead},
                    Cell::Alive => if alives >1 && alives < 4  {Cell::Alive} else {Cell::Dead},
                }
            }).collect()
        }).collect();
    }
...
fn main() {
    ...
    if args.len() <= 1 {
        filename.insert_str(0, "input.txt");
    }
    else {
        filename.insert_str(0, args[1].as_str());
    }
    ...
    for _ in 0..20{
        ...
        for i in 0..life.size[0]{
            for j in 0..life.size[1]{
                print!("{}", if life.field[i][j]==Cell::Alive {'#'} else {'.'});
            }
            println!();
        }
        println!();
